...
case "${PACKAGE_MANAGER}" in
    ...
    zypper)
        ...
        ;;
    pacman)
        HEADERS_INSTALLED=false # Core runtime packages in Arch include development headers
        ;;
esac
...
if [ "${VERSION}" = "latest" ]; then
    # Track the location header from GitHub releases redirect to grab the exact clean version string
